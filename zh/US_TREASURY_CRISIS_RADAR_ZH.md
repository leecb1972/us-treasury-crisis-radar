---
lang: zh-CN
title: 美国美债危机雷达 · 2026--2030
viewport: width=device-width,initial-scale=1
---

::: wrap
::: lang
[中文](../zh/)[English](../en/)

::: eyebrow
US TREASURY CRISIS RADAR · 2026--2030

# 不要预测"美债什么时候崩"

::: sub
真正应该监测的是：美国什么时候从**"有能力发行大量债务"**进入**"必须付出越来越高的价格才能发行新增债务"**。

::: notice
**核心原则：**40万亿美元以上的债务本身不是危机信号。雷达关注**融资价格 +
期限溢价 + 财政现金流 + 经济增长 + 边际需求**是否同时恶化。

::: grid
::: card
::: label
10年期美债收益率

::: metric
4.8%

::: status
[]{.dot .yellow}监测

::: card
::: label
期限溢价

::: metric
0.6%

::: status
[]{.dot .green}正常

::: card
::: label
利息 / 财政收入

::: metric
18%

::: status
[]{.dot .yellow}财政压力

::: card
::: label
g − r

::: metric
+0.3%

::: status
[]{.dot .yellow}接近临界

::: card
::: label
外国需求

::: metric
稳定

::: status
[]{.dot .green}正常

::: card
::: label
弗格森极限

::: metric
Crossed

::: status
[]{.dot .purple}战略信号

::: panel
::: label
综合压力指数（示例基线，非实时读数）

::: score
32[ / 100]{style="font-size:20px;color:var(--muted)"}

::: bar
::: fill

**Level 1 · Fiscal 财政压力** ---
债务与利息负担值得持续监测，但不能据此判断"美债危机已经发生"。

::: tabs

::: panel
## 核心监测框架

  指标                  为什么重要                     绿色      黄色           橙色                红色
  --------------------- ------------------------------ --------- -------------- ------------------- ---------------------
  **10Y Yield**         长期融资价格                   \<4.5%    4.5--5.0%      5.0--5.5%           \>5.5%
  **期限溢价**          投资者要求的期限风险补偿       \<0.5%    0.5--1.0%      1.0--1.5%           \>1.5% + rapid rise
  **利息 / 财政收入**   联邦收入中被净利息支出的占比   \<15%     15--20%        20--25%             \>25%
  **g − r**             名义增长是否跑赢有效融资成本   \>+1%     0--1%          −1--0%              \<−1%
  **外国需求**          全球边际美债需求是否恶化       稳定/up   Mild decline   Sustained decline   Rapid net outflow

这些是分析型预警区间，不是官方危机界线。趋势、持续时间和指标共振比单一读数更重要。

::: panel
## "危机三角形"

::: triangle
**期限溢价 ↑** + **g − r ↓** + **利息 / 财政收入 ↑**

如果三者同时恶化，问题就从"债务很多"转向"新增债务的边际价格越来越高"。

## 弗格森极限 --- strategic-fiscal indicator

Historian Niall Ferguson proposed "Ferguson's Law": when a great power
spends more on debt servicing than on defense, it risks ceasing to be a
great power. The **弗格森极限** is the point at which interest payments
exceed defense spending.

它适合作为**国家能力 /
财政容量信号**，但不是美债违约风险阈值，也不能机械地解释为"衰退已经开始"。

::: panel
## 2026--2030监测表

  指标                  Frequency            Displayed value                                Status                                  Source
  --------------------- -------------------- ---------------------------------------------- --------------------------------------- -------------------------------------------------------------------------------------------------------------
  债务余额              每日                 Loading...                                     [规模监测]{.badge .byellow}             [Treasury Fiscal Data](https://fiscaldata.treasury.gov/datasets/debt-to-the-penny/){target="_blank"}
  10Y Treasury          每日                 4.8% (illustrative)                            [监测]{.badge .byellow}                 [美国财政部](https://home.treasury.gov/resource-center/data-chart-center/interest-rates){target="_blank"}
  期限溢价              每日                 0.6% (illustrative)                            [正常]{.badge .bgreen}                  [纽约联储](https://www.newyorkfed.org/research/data_indicators/term-premia-tabs){target="_blank"}
  Net 利息 / 财政收入   月度/季度            18% (illustrative)                             [财政压力]{.badge .byellow}             [财政部月度财务报表](https://fiscaldata.treasury.gov/datasets/monthly-treasury-statement/){target="_blank"}
  债务 / GDP            季度                 \~100%+ baseline                               [Long-term pressure]{.badge .borange}   [CBO](https://www.cbo.gov/publication/62105){target="_blank"}
  初级赤字 / GDP        季度                 基准情景监测                                   [监测]{.badge .byellow}                 [CBO](https://www.cbo.gov/publication/62105){target="_blank"}
  外国美债需求          每月                 需要结合TIC判断                                [监测]{.badge .byellow}                 [Treasury TIC](https://home.treasury.gov/data/treasury-international-capital-tic-system){target="_blank"}
  弗格森极限            Fiscal / strategic   Crossed in 2024 under Ferguson\'s definition   [战略信号]{.badge .bpurple}             [Hoover Institution](https://www.hoover.org/research/fergusons-law){target="_blank"}

刷新美债余额

Treasury Debt to the
Penny可在浏览器中实时抓取。其他指标在多源数据管道完成前，均明确标为示例/参考值。

::: panel
## 不要预测"美债什么时候崩"：真正应该监测融资价格

### 摘要

Discussion of 美国财政部 debt often falls into two extremes: one expects
a sudden collapse after debt reaches some number; the other argues that
the United States can borrow indefinitely because it issues the world's
dominant reserve currency. A more useful question is not "when will
Treasuries collapse?" but when the United States moves from being able
to issue large amounts of debt to having to pay increasingly higher
prices to issue new debt.

### 1. 40万亿美元以上不是危机本身

债务总额是存量指标，真正的压力通过融资条件体现。也不能把全部未偿债务直接乘以当前10年期收益率，因为美债具有不同期限、票息和再融资时间表。

### 2. 美国的约束是实际资源，而不只是美元数量

美元发行能力是美国的巨大优势，但政府不能凭空创造石油、芯片、电力、住房、工程师和生产率。持续财政扩张最终可能通过通胀、长期利率、期限溢价或金融压制表现出来。

### 3. 为什么期限溢价特别重要

10年期收益率上升并不自动意味着财政危机。它可能反映更强的增长、更高的通胀或未来短端利率预期。更值得警惕的是经济走弱或货币政策放松，而长端收益率仍然高企、期限溢价继续上升。

### 4. g − r：经济能否跑赢债务成本？

一个简化的债务动态关系是
`Δ(Debt/GDP) ≈ Primary Deficit + (r − g) × Debt/GDP`.
如果名义增长长期低于有效融资成本，同时初级赤字持续存在，稳定债务就会越来越困难。

### 5. 利息 / 财政收入: how much future fiscal space is already committed?

净利息支出占财政收入的比例非常直观：政府每收到1美元，有多少已经被过去借债的成本占用？

### 6. 外国投资者是否真的离场？

单一国家的美债持仓变化并不等于全球抛售。应区分官方储备与私人资本，也要区分美债持仓与更广义的美元资产。重点观察长期、持续的边际需求变化。

### 7. 弗格森极限: a different question

Niall Ferguson\'s "Ferguson's Law" adds a strategic-fiscal lens. His
formulation says that when debt-servicing costs exceed defense spending,
a great power risks ceasing to be a great power. He calls that crossing
point the **弗格森极限**. The name refers to 18th-century Scottish
philosopher Adam Ferguson, not Niall Ferguson himself.

The 弗格森极限 should therefore be treated as a **strategic capacity
warning**, not as a technical default trigger. It asks whether debt
service is beginning to compete with a core component of national power.
The Treasury Crisis Radar asks a different question: whether the
marginal price of new borrowing is rising enough to constrain fiscal and
monetary choices.

### 8. 五个信号值得高度警惕

**10Y yield ↑ + 期限溢价 ↑ + Interest/Revenue ↑ + g−r ↓ + foreign
marginal demand ↓** is more informative than any single indicator.

### 9. 五阶段框架

**第一阶段：**可以大量借债且成本较低；**第二阶段：**仍能借但价格越来越高；**第三阶段：**利息成本开始明显挤压财政空间；**第四阶段：**财政主导风险上升；**第五阶段：**可能通过财政整顿、金融压制、通胀、更高名义增长或多种方式组合完成调整。

### 结论

> 真正值得等待的不是"崩盘日"，而是债务边际价格持续上升，并且财政承受能力同步恶化的拐点。

::: panel
## 方法论

1.  **Debt stock:** 债务有多大？
2.  **Fiscal flow:** 财政赤字是否继续创造新增债务？
3.  **Financing price:** 10年/30年期收益率是否持续上升？
4.  **Risk compensation:** 期限溢价是否上升？
5.  **Growth differential:** `g−r`是否转负？
6.  **Fiscal capacity:** 利息/财政收入是否持续上升？
7.  **Marginal demand:** 外国及私人投资者是否降低长期美债边际需求？
8.  **Strategic capacity:** has the 弗格森极限 been crossed, and is the
    gap widening?

`Illustrative Score = 20% 期限溢价 + 20% g−r + 20% Interest/Revenue + 20% Fiscal Deficit + 20% Market Demand`

The score is a research framework, not an official risk measure or
investment signal. 弗格森极限 is kept outside the composite because it
measures strategic-fiscal capacity rather than marginal Treasury
pricing.

::: panel
## 权威数据源

-   [美国财政部 Fiscal
    Data](https://fiscaldata.treasury.gov/){target="_blank"} --- debt,
    rates and fiscal data.
-   [CBO 2026--2036 Budget and Economic
    Outlook](https://www.cbo.gov/publication/62105){target="_blank"} ---
    deficit, debt and interest baseline. CBO projects debt held by the
    public rising from 101% of GDP in 2026 to 120% in 2036, with net
    interest rising from 3.3% to 4.6% of GDP.
-   [纽约联储 Treasury
    期限溢价](https://www.newyorkfed.org/research/data_indicators/term-premia-tabs){target="_blank"}
    --- term-premium estimates.
-   [Treasury
    TIC](https://home.treasury.gov/data/treasury-international-capital-tic-system){target="_blank"}
    --- foreign portfolio flows and holdings.
-   [Hoover Institution --- Ferguson's
    Law](https://www.hoover.org/research/fergusons-law){target="_blank"}
    --- primary institutional source for Ferguson's Law and the
    弗格森极限.
-   [Ferguson's Law working
    paper](https://www.hoover.org/sites/default/files/research/docs/fergusons-law.pdf){target="_blank"}
    --- debt service, military spending and fiscal limits of power.

::: footer
美国财政部 Crisis Radar · 研究版 · 2026-09-06 ·
仅用于研究与教育，不构成投资建议。
